...
def colar1488(image: Image.Image, color_hex: str) -> Image.Image:
    try:
    ...
    try:
        with Image.open(image_path) as img:
            colored_img = await loop.run_in_executor(
                thread_pool,
                lambda: colar1488(img, color_hex)
            )
        ...
        print(f"ошибко обработки изображения {image_path.name}: {str(e)}")
...
async def process_all_images(color_hex: str) -> bytes:
    ...
    listva_dir = Path("painting/photo/listva")
    png_files = [f for f in listva_dir.glob('*.png') if f.is_file()]
    
    if not png_files:
        raise ValueError("в папке listva не найдено PNG файлов")
...
def get_listva_keyboard():
    builder = InlineKeyboardBuilder()
    builder.button(text="🎨 Палитра HEX цветов", url="https://csscolor.ru/")
    builder.adjust(1)
    return builder.as_markup()

@router.message(Command("listva"))
async def listva(message: Message, bot: Bot):
    ...
    if len(args) == 1:
        msg = await message.answer(
            f"<b>чтоб покрасить листву введи -</b> <code>/listva #FFFFFF</code>\n\n",
            reply_markup=get_listva_keyboard(),
            parse_mode="HTML"
        )
        return
    ...
    try:
        ...
        result_file = BufferedInputFile(zip_data, filename=f"listva.zip")
        
        await message.reply_document(
            document=result_file,
            caption=f"<b>📊 файл готов</b>\n<b>📝 назва - listva.zip</b>",
            parse_mode="HTML"
        )
